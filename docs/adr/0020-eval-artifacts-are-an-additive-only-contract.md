...
`fastskill` reads all three. Its `eval score` subcommand re-derives verdicts from a completed run directory, and a second repository holds the fixtures that pin it: `gofastskill/skill` keeps two recorded runs committed at `evals/fixtures/pass/` and `evals/fixtures/fail/`, and its `skill-evals.yml` workflow scores both on every push, asserting the first exits zero and the second exits non-zero. Those fixtures are checked in. They are not regenerated on each run, and they cannot be: regenerating them needs a live agent and a paid provider call, which is the thing the fixtures exist to avoid.

So the consumer chain is two links long, and the far end is a different repository on a different release cadence. Nothing in `aikit-evals` CI reads those bytes, which is exactly why the schema rule has to be a rule rather than a test.
...
- The artifact structs accrete. `TrialResult` gains `exit_code`, `terminal`, `cost_usd`, `skill_path` and a full token breakdown in this change, and it will keep growing. That is the intended cost: a struct that is slightly untidy is cheaper than a consumer that silently reports the wrong number.
- The pass and fail fixtures committed in `gofastskill/skill` keep scoring identically across an `aikit-evals` bump. This is the property the rule exists to protect, and it is testable in the repository that owns them: score both fixtures after the bump and the verdicts must not move.
