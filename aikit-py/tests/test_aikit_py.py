...
Description here."""
    ...
    expected_skill_md_path = os.path.join(temp_project_root, ".cursor/skills/my-skill/SKILL.md")
    assert path == expected_skill_md_path
    assert os.path.exists(os.path.join(temp_project_root, ".cursor/skills/my-skill/scripts/setup.sh"))
    assert os.path.exists(os.path.join(temp_project_root, ".cursor/skills/my-skill/scripts/cleanup.sh"))

    with open(path, "r") as f:
        assert f.read() == skill_md

    with open(os.path.join(temp_project_root, ".cursor/skills/my-skill/scripts/setup.sh"), "rb") as f:
        assert f.read() == b"#!/bin/sh\necho 'setup'"

    with open(os.path.join(temp_project_root, ".cursor/skills/my-skill/scripts/cleanup.sh"), "rb") as f:
        assert f.read() == b"#!/bin/sh\necho 'cleanup'"
