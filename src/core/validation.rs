...
pub fn validate_package_name(name: &str) -> Result<(), AikError> {
    // Check length
    if name.is_empty() {
        return Err(AikError::InvalidSource("Package name cannot be empty".to_string()));
    }

    if name.len() > 50 {
        return Err(AikError::InvalidSource("Package name too long (max 50 characters)".to_string()));
    }

    // Check characters (alphanumeric, hyphens, underscores only)
    let valid_chars = Regex::new(r"^[a-zA-Z0-9_-]+$").map_err(|_| AikError::Generic("Invalid regex".to_string()))?;
    ...
    if reserved.contains(&name.to_lowercase().as_str()) {
        return Err(AikError::InvalidSource(format!("'{}' is a reserved name", name)));
    }
...
pub fn validate_version_format(version: &str) -> Result<(), AikError> {
    ...
    let semver_regex = Regex::new(r"^v?\d+\.\d+\.\d+$").map_err(|_| AikError::Generic("Invalid regex".to_string()))?;
...
pub fn sanitize_path(path: &str) -> Result<PathBuf, AikError> {
    ...
    if canonical.is_absolute() && !canonical.starts_with(std::env::current_dir()?) {
        return Err(AikError::InvalidSource("Path must be within current working directory".to_string()));
    }
...
pub fn validate_local_path(path: &Path) -> Result<(), AikError> {
    ...
    if !path.exists() {
        return Err(AikError::InvalidSource(format!("Path '{}' does not exist", path.display())));
    }

    // Check if it's a directory
    if !path.is_dir() {
        return Err(AikError::InvalidSource(format!("Path '{}' is not a directory", path.display())));
    }
...
pub fn validate_github_repo_name(name: &str) -> Result<(), AikError> {
    if name.is_empty() {
        return Err(AikError::InvalidGitHubUrl("Repository name cannot be empty".to_string()));
    }

    if name.len() > 100 {
        return Err(AikError::InvalidGitHubUrl("Repository name too long".to_string()));
    }

    // GitHub repo names can contain letters, numbers, hyphens, underscores, and periods
    let valid_repo = Regex::new(r"^[a-zA-Z0-9_.-]+$").map_err(|_| AikError::Generic("Invalid regex".to_string()))?;
    if !valid_repo.is_match(name) {
        return Err(AikError::InvalidGitHubUrl(
            "Repository name can only contain letters, numbers, hyphens, underscores, and periods".to_string()
        ));
    }
...
pub fn validate_github_owner_name(name: &str) -> Result<(), AikError> {
    if name.is_empty() {
        return Err(AikError::InvalidGitHubUrl("Owner name cannot be empty".to_string()));
    }

    if name.len() > 39 { // GitHub's limit
        return Err(AikError::InvalidGitHubUrl("Owner name too long".to_string()));
    }
    ...
    if !valid_owner.is_match(name) {
        return Err(AikError::InvalidGitHubUrl(
            "Owner name can only contain letters, numbers, and hyphens (no consecutive hyphens)".to_string()
        ));
    }
